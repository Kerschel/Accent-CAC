...
def filter_df(df):
    ...
    tobago = df[df['HomeCountry'] == 'Tobago'][:80]
    barbados = df[df['HomeCountry'] == 'Barbados'][:80]
    lucia = df[df['HomeCountry'] == 'St. Lucia'][:80]
    ...
    df = (trinidad)
    df = df.append(tobago)
    df = df.append(barbados)
    df = df.append(lucia)

    return df

def split_people(df,test_size=.15):
    '''
    ...
    '''
    return train_test_split(df['Filename'],df['HomeCountry'],test_size=test_size)
